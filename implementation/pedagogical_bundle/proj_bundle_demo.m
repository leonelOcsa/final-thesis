...
args1 = cell(1,7); 
args1{1} = m; %number of cameras
args1{2} = n; %number of points (hm, actually contained in str)
args1{3} = imgs; %image points, first row is the index
%args1{4} = mot_n0; %no motion as argument, (it's already in init sol.) 
args1{5} = str; %the structure, needed in example 1 only
%args1{6} = focals; %we don't use focal lengthes in projective bundle
...
[proj_mot_vec_opt_1, err1, l1, C_vals1]=optimise_fun(
                'err_proj_mot',       %error function
                'jac_proj_mot',       %corresponding jacobian
                [],                   %possibility for a specific updt function
                args1,                %arguments
...
[proj_mot_str_vec_opt_2, err2, l2, C_vals2]=optimise_fun(
                'err_proj_mot_str', 
