function [mot, new_imgs] = reconst_affine_last_equal_smooth(imgs,pair_mat, r_f)
...
new_imgs = cell(m,1); %for k = 1:m, new_imgs{k} = []; end 
end
for j = 2:m, %view 2
for i =1:j-1, %view 1
if pair_mat(i,j)==1,
...
%tic
    ...
    StS(rngj, rngj) = StS(rngj,rngj)+block2'*block2;

    StS(rngi,rngi) = StS(rngi,rngi)+block1'*block1;

    StS(rngi,rngj) = StS(rngi,rngj)+block1'*block2;
    
    StS(rngj,rngi) = StS(rngj,rngi)+block2'*block1;

%%%%%%%%%% StSe
     StSe([rngi rngj],4) = StSe([rngi rngj],4) - F(3,3)*[block1';block2'];
...
%%regularise with continuity
cont_reg = eye(size(StS)) - diag(ones(1,size(StS,1)-2),2)...
-diag(ones(1,2),2-size(StS,1));

%%regularise with camera orientation vertical

%keyboard
alpha = 0.1
StS = StS + alpha*cont_reg'*cont_reg;
